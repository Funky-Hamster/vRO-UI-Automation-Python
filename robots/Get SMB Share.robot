...
Variables    ../../PageObjects/PowerScale/get_smb_sharepo.py
...
*** Keywords ***
Workflow Get SMB Share Basic
    [Arguments]    &{get_smb_share_data}
    Sleep    10
    Click Element    xpath=//a[text()=""]
    Sleep    10
    Wait Until Element Is Visible    id=input_clusterName
    Select From List By Label    id=input_clusterName    ${get_smb_share_data.cluster}
    Sleep    10
    Wait Until Element Is Visible    id=input_zone
    Select From List By Label    id=input_zone    ${get_smb_share_data.access_zone}
    Sleep    10
    Wait Until Element Is Visible    id=input_smbShareName
    Select From List By Label    id=input_smbShareName    ${get_smb_share_data.smb_share_name}
    No Errors
...
Validate Workflow Get SMB Share Basic
    [Arguments]    &{get_smb_share_data}
    ...
    Wait Until Element Is Enabled    xpath=//label[@title="${get_smb_share_data.cluster}"]/parent::button/parent::div/parent::div/button[@type="button"]
    Click Button    xpath=//label[@title="${get_smb_share_data.cluster}"]/parent::button/parent::div/parent::div/button[@type="button"]
    ...
    Wait Until Element Is Enabled    xpath=//label[@title="${get_smb_share_data.access_zone} : ${get_smb_share_data.name}"]/parent::button
    Click Button    xpath=//label[@title="${get_smb_share_data.access_zone} : ${get_smb_share_data.name}"]/parent::button
    ${description}    Get Text  xpath=//th[text()="description"]/following-sibling::td
    Should Be Equal    ${description}    ${get_smb_share_data.description}
    ${path}    Get Text  xpath=//th[text()="path"]/following-sibling::td
    Should Be Equal    ${path}    ${get_smb_share_data.path}
    ${zone}    Get Text  xpath=//th[text()="zone"]/following-sibling::td
    Should Be Equal    ${zone}    ${get_smb_share_data.access_zone}
    ${name}    Get Text  xpath=//th[text()="name"]/following-sibling::td
    Should Be Equal    ${name}    ${get_smb_share_data.name}
